...
public class ShiroConfig {
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(DefaultWebSecurityManager defaultWebSecurityManager) {
        ...
        filterChainDefinitionMap.put("/user/login", "anon"); // anon: 相当于系统的公认资源，所有用户都可以访问，无需认证授权
        filterChainDefinitionMap.put("/user/register", "anon");
        filterChainDefinitionMap.put("/register.jsp", "anon");
        filterChainDefinitionMap.put("/user/getImage", "anon");
        // /**表示除了/login.jsp以外的所有页面都需要认证授权
        filterChainDefinitionMap.put("/**", "authc"); // authc: 请求这个资源需要认证和授权

        // 默认的认证界面的路径
        shiroFilterFactoryBean.setLoginUrl("/login.jsp"); // 不写，默认也是/login.jsp
